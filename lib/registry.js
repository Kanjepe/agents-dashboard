...
import { SKILL_BLACKLIST, dayKey } from './utils.js';
...
// Where to look for per-project .claude/skills + .claude/agents folders.
// Override with PROJECTS_ROOT env var (e.g. ~/Code, ~/dev, D:\repos).
// If unset, defaults to ~/Projects which may not exist on every machine —
// the projects tab will simply be empty in that case (no error).
const USER_PROJECTS_ROOT = process.env.PROJECTS_ROOT || join(homedir(), 'Projects');

const PROJECT_SCAN_DEPTH = Number(process.env.PROJECTS_SCAN_DEPTH) || 6;
...
export function parseFrontmatter(content) {
  ...
  try {
    ...
    // Fall through to regex fallback below. Common cause: a single-line
    // `description:` containing `: ` (e.g. `Examples: <example>Context: ...`)
    // which YAML mis-reads as a nested mapping. We still want the top-level
    // fields so the dashboard can show name + description.
  }
  if (!yamlOk) {
