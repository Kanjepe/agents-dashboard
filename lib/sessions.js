...
import { SKILL_BLACKLIST, decodeProjectDirName, shortProjectName } from './utils.js';
...
export function aggregateSession(sessionId, entries, fileStats) {
  ...
  for (const entry of entries) {
    ...
    if (entry.type === 'assistant' && entry.message) {
      ...
      if (usage) {
        ...
        tokens.cacheCreate += cCreate;
        if (entry.timestamp) {
          messageTokens.push({
            t: new Date(entry.timestamp).getTime(),
            tokens: inTok + outTok + cRead + cCreate,
          });
        }
      }
  ...
  const tokensLast5Min = recentMessageTokens
    .filter((m) => now - m.t < BURN_WINDOW_MS)
    .reduce((s, m) => s + m.tokens, 0);
  const costUsd = estimateCostUsd(tokens, model);
...
function freshenSession(session) {
  ...
  const tokensLast5Min = (session._messageTokens || [])
    .filter((m) => now - m.t < BURN_WINDOW_MS)
    .reduce((s, m) => s + m.tokens, 0);
  return {
    ...session,
