...
import { estimateCostUsd, getPricingTable, modelLabel } from './pricing.js';
...
export async function aggregateStats() {
  ...
  const topBy = (key, costKey, limit = 8) =>
    Array.from(byProject.entries())
      .map(([name, t]) => ({ name, tokens: t[key], cost: t[costKey] || 0, isMemory: !!t.isMemory }))
      .filter((p) => p.tokens > 0)
  ...
  const memoryBetween = (fromKey, toKey) => {
    ...
    return { tokens, cost: Math.round(cost * 100) / 100 };
  };
